...
router.get('/other/:userId', async (req, res) => {
  try {
    ...
    if (userId != '') {
      const docsArray = [];
      const ownDocs = dbDocsList
        .where('userReceiveID', 'array-contains', userId)
        .get()
        .then((snapshot) => {
          snapshot.forEach((doc) => {
            const docData = doc.data();
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    console.log(docId);
    if (docId != '') {
      const query = dbDocsList
        .where('filename', '==', docId)
        .get()
        .then((querySnapshot) => {
          if (querySnapshot.size > 0) {
            querySnapshot.forEach((doc) => {
              if (doc.data().userCreateID == req.user.user_id) {
                // đúng chủ sỡ hữu thì mới đc xóa
                doc.ref.delete();
              }
            });
            return res.status(200).json({
...
router.post('/sign', async (req, res) => {
      ...
      const updateDocs = dbDocsList
        ...
        .then((snapshot) => {
          snapshot.forEach(async (document) => {
            ...
            if (isFinish == true) {
              const documentRef = serviceAccount
                .firestore()
                .collection('docslist')
                .doc(document.id); // Replace with your own collection name and document ID
              documentRef
                .update({ status: 1, isComplete: docData.isComplete })
                .then(() => {
              ...
              const documentRef = serviceAccount
                ...
                .doc(document.id); // Replace with your own collection name and document ID
              documentRef
                .update({ isComplete: docData.isComplete })
                .then(() => {
  ...
  file
    ...
    .then(async (data) => {
      ...
      https
        .request(imageName, async function (response) {
      ...
      https
        .request(imageName, async function (response) {
          ...
          response.on('end', async function () {
            ...
            return res.status(200).json({
              imageWidth:
                signatureImageWidth >= 200 ? 200 : signatureImageWidth,
              imageHeight:
                signatureImageHeight >= 200 ? 150 : signatureImageHeight,
              message: 'success',
              imgUrl: imageName,
            });
          });
